...
int main(void) {
  ...
  close(fd);
  free(filename);
  char *command; asprintf(&command, "/usr/bin/cc -Wall -Werror -Wextra -Wpedantic Sully_%d.c -o Sully_%d && ./Sully_%d", i, i, i);
  system(command);
  free(command);
}
